...
import org.dreambot.behaviour.Filling;
...
import org.dreambot.settings.script.FillMode;
...
@ScriptManifest(category = Category.MONEYMAKING, name = "cCFillerFarm", author = "camalCase", version = 0.0)
public class Filler extends AbstractScript implements PaintInfo, HumanMouseListener, ChatListener, ItemContainerListener, SpawnListener {
    ...
    public void onStart(String... params) {
        if (params != null) {
            FillMode mode = Arrays.stream(FillMode.values()).filter(x -> x.toString().equalsIgnoreCase(params[0]))
                    .findFirst()
                    .orElse(null);
            if (mode != null) {
                ScriptSettings.getSettingsData().fillMode = mode;
            }
        }
    ...
    private void init() {
        ...
        Fractal muleOff = ScriptSettings.getSettingsData().itemMuleOff ? new ItemMuleOff().setSimpleName("Item mule off") : new MuleOff().setSimpleName("Mule off");
        tree.setSimpleName("cCFillerFarm")
                .addChildren(
                        new TutorialTree().setSimpleName("Tutorial island"),
                        new LampHandler().setSimpleName("lamp handler"),
                        new MixedCombat(() -> !ScriptSettings.getSettingsData().alreadyUnrestriced
                                && (Combat.getCombatLevel() < 20 || Skills.getRealLevel(Skill.HITPOINTS) < 25)
                                && Quests.getQuestPoints() < 10)
                                .setSimpleName("Melee training for quests"),
                        new ShuffleFractal(() -> Quests.getQuestPoints() < 10 && !ScriptSettings.getSettingsData().alreadyUnrestriced).setSimpleName("Questin")
                                .addChildren(
                        ...
                        new TotalLevel(() -> !ScriptSettings.getSettingsData().alreadyUnrestriced && Skills.getTotalLevel() < 100).setSimpleName("Get 100 total"),
                        muleOff,
                        new Filling(() -> true).setSimpleName("Fill")
...
//        new AIAntiban();
    ...
    public int onLoop() {
        ...
        if (ScriptSettings.getSettingsData().ignorePlaytime && Skills.getTotalLevel() >= 100 && Quests.getQuestPoints() >= 10) {
...
//        }
    ...
    public String[] getPaintInfo() {
        ...
        if (MuleOff.timer != null) {
            muleOff = formatTime(MuleOff.timer.remaining());
        } else {
            if (ItemMuleOff.timer != null) muleOff = formatTime(ItemMuleOff.timer.remaining());
    ...
    public static boolean isUnrestricted() {
        return ScriptSettings.getSettingsData().alreadyUnrestriced
                || (Skills.getTotalLevel() >= 100 && Quests.getQuestPoints() >= 10 && Varcs.getInt(PLAY_TIME_VARCINT) >= 1200);
    }
    ...
    public void onInventoryItemChanged(Item incoming, Item existing) {
        if (existing.getID() == ScriptSettings.getSettingsData().fillMode.getId()) {
            grossGp += (incoming.getLivePrice() - existing.getLivePrice());
        }
    }
}
