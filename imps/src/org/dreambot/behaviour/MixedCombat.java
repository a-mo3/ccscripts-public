...
public class MixedCombat extends Fractal {
    ...
    public MixedCombat(Supplier<Boolean> acceptCondition) {
        ...
        Supplier<NPC> varrockCowSupplier = () -> NPCs.closest(x -> x.getName().equals("Cow") && !x.isInCombat()
                && x.hasAction("Attack") && x.canReach() && VARROCK_COWS.contains(x));

        Supplier<NPC> lummyCowSupplier = () -> NPCs.closest(x -> x.getName().equals("Cow") && !x.isInCombat()
                && x.hasAction("Attack") && x.canReach() && COW_PEN_LUMMY.contains(x));

        Supplier<NPC> lummyGoblinSupplier = () -> NPCs.closest(x -> x.getName().contains("Goblin") && !x.isInCombat()
