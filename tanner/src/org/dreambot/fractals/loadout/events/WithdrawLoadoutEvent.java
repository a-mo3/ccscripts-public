...
//        }
    ...
    private void buyItems(List<LoadoutItem> itemsToBuy) {
        BuyItemsEvent.Response response = new BuyItemsEvent(itemsToBuy).executed();
        ...
        if (response == BuyItemsEvent.Response.NO_GP && muleRequiredGP) {
            ...
            if (inventoryLoadout.getSellItems() != null && inventoryLoadout.getSellItems().length > 0
                    && OwnedItems.containsAny(inventoryLoadout.getSellItems())) {

                Logger.info("selling all items");
                new SellAllEvent(inventoryLoadout.getSellItems()).execute();
                return;
            }
