...
import org.dreambot.behaviour.training.MixItems;
...
import org.dreambot.scriptdata.CraftingModes;
import org.dreambot.scriptdata.FtpCraftingSettings;
...
public class FtpCrafting extends PseudoScript implements ItemContainerListener {
    ...
    public void onStart(String... args) {
        for (String arg : args) {
            CraftingModes mode = Arrays.stream(CraftingModes.values()).filter(x -> x.name().equalsIgnoreCase(arg))
                    .findFirst().orElse(null);
            if (mode != null) {
                FtpCraftingSettings.getData().forceCrafting = mode;
            }
        }
        init();
    }
    ...
    @Override
    public void init() {
        SettingsLoader.changePath("cCCraftingFarm");
        Client.getInstance().addEventListener(this);


        MuleRequestEvent.moneyMakeMode = FtpCraftingSettings.getData().noMuleMode;
        ...
        tree.setSimpleName("cCCraftingFarm");
        tree.addChildren(
                new TutorialTree().setSimpleName("Tutorial island"),
                new LampHandler().setSimpleName("lamp handler"),
                new GetMoneyBranch(() -> MuleRequestEvent.moneyMakeMode).setSimpleName("Get money"),
                ...
                new MuleOff(FtpCraftingSettings.getData().hoursUntilMuleOff, FtpCraftingSettings.getData().moneyLeftAfterMuling).setSimpleName("Mule Off"),
                new MixItems(() -> Skills.getRealLevel(Skill.CRAFTING) < 5, ItemID.LEATHER_GLOVES, ItemID.LEATHER, ItemID.NEEDLE)
                        .setInventoryLoadout(new InventoryLoadout()
                                .addItem(ItemID.LEATHER, 26)
                                .setRefill(52)
                                .addItem(ItemID.NEEDLE)
                                .addItem(ItemID.THREAD, 52)
                                .setRefill(100)
                        )
                        .setSimpleName("Leather gloves till lvl 5"),

                FtpCraftingSettings.getData().forceCrafting.getMethod(),

                CraftingModes.RUBY_NECKLACES.getMethod(),
                CraftingModes.EMERALD_AMULETS_U.getMethod(),
                CraftingModes.SAPPHIRE_AMULETS_U.getMethod(),
                CraftingModes.SAPPHIRE_RINGS.getMethod(),
                CraftingModes.SAPPHIRE_NECKLACES.getMethod(),
                CraftingModes.SAPPHIRE_RINGS.getMethod(),
                CraftingModes.GOLD_AMULETS_U.getMethod(),
                CraftingModes.GOLD_NECKLACES.getMethod(),
                CraftingModes.GOLD_RINGS.getMethod()
        );
//        new AIAntiban();
    }
    ...
    @Override
    public int onLoop() {
        FtpCraftingSettings.FtpCraftingData settings = FtpCraftingSettings.getData();
        ...
        if (FtpCraftingSettings.getData().ignorePlaytime && Skills.getTotalLevel() >= 100 && Quests.getQuestPoints() >= 10) {
        ...
        if (!checkedPlayTime && MyVarps.getTutVarp() >= 1000) {
            ...
            if (Tabs.open(Tab.QUEST)) {
                ...
                if (playedMins > 1) {
                    Logger.info("Set runtime Mins: " + playedMins);
...
//
    ...
    public String[] getPaintInfo() {
        ...
        return new String[]{
                "cCCraftingFarm: " + runtime.formatTime(),
...
//        grossGp += (incoming.getLivePrice() + 1) * quantity;
    ...
    public void onMouseClicked(MouseEvent e) {
        ...
        if (Gui.wasButtonClicked(e.getPoint())) {
            SwingUtilities.invokeLater(() -> new Gui(FtpCraftingSettings.getData()));
        }
    }
}
