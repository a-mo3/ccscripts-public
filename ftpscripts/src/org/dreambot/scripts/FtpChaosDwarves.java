...
import org.dreambot.behaviour.method.chaosdwarves.ChaosDwarves;
...
import org.dreambot.scriptdata.FtpChaosDwarvesSettings;
...
public class FtpChaosDwarves extends PseudoScript implements ItemContainerListener {
    ...
    @Override
    public void init() {
        SettingsLoader.changePath("cCChaosDwarvesFarm");
        Client.getInstance().addEventListener(this);


        MuleRequestEvent.moneyMakeMode = FtpChaosDwarvesSettings.getData().noMuleMode;
...
//                ItemID.WATER_RUNE,
        ...
        tree.setSimpleName("cCChaosDwarvesFarm");
        tree.addChildren(
                new TutorialTree().setSimpleName("Tutorial island"),
                new LampHandler().setSimpleName("lamp handler"),
                new GetMoneyBranch(() -> FtpChaosDwarvesSettings.getData().noMuleMode).setSimpleName("Get money"),
                ...
                new MuleOff(FtpChaosDwarvesSettings.getData().hoursUntilMuleOff, FtpChaosDwarvesSettings.getData().moneyLeftAfterMuling)
                        .setSimpleName("Mule Off"),
                new MixedCombat(() -> Combat.getCombatLevel() < FtpChaosDwarvesSettings.getData().minCombatLevel)
                        .setSimpleName("Get higher combats"),
                new F2PMagicBranch(() -> Skills.getRealLevel(Skill.MAGIC) < 25)
                        .setSimpleName("Magic to 25 to unlock teleport to varrock"),
                new ChaosDwarves(() -> true)
        );
//        new AIAntiban();
    }
    ...
    @Override
    public int onLoop() {
        FtpChaosDwarvesSettings.Data settings = FtpChaosDwarvesSettings.getData();
        ...
        if (FtpChaosDwarvesSettings.getData().ignorePlaytime && Skills.getTotalLevel() >= 100 && Quests.getQuestPoints() >= 10) {
...
//
    ...
    public String[] getPaintInfo() {
        ...
        return new String[]{
                "cCChaosDwarvesFarm: " + runtime.formatTime(),
    ...
    public void onMouseClicked(MouseEvent e) {
        ...
        if (Gui.wasButtonClicked(e.getPoint())) {
            SwingUtilities.invokeLater(() -> new Gui(FtpChaosDwarvesSettings.getData()));
        }
    }
}
