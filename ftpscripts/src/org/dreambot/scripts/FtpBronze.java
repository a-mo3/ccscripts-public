...
import org.dreambot.behaviour.SmeltBars;
...
import org.dreambot.scriptdata.FtpBronzeSettings;
...
public class FtpBronze extends PseudoScript implements ItemContainerListener {
    ...
    @Override
    public void init() {
        SettingsLoader.changePath("cCBronzeFarm");
        ...
        MuleRequestEvent.moneyMakeMode = FtpBronzeSettings.getData().noMuleMode;

        MuleOff.LOOT = new int[]{
                ItemID.BRONZE_BAR
        };

        tree.setSimpleName("cCBronzeFarm");
        tree.addChildren(
                new TutorialTree().setSimpleName("Tutorial island"),
                new LampHandler().setSimpleName("lamp handler"),
                new GetMoneyBranch(() -> FtpBronzeSettings.getData().noMuleMode).setSimpleName("Get money"),
                ...
                new MuleOff(FtpBronzeSettings.getData().hoursUntilMuleOff, FtpBronzeSettings.getData().moneyLeftAfterMuling)
                        .setSimpleName("Mule Off"),

                new SmeltBars(() -> true, ItemID.BRONZE_BAR)
...
//        new AIAntiban();
    ...
    @Override
    public int onLoop() {
        FtpBronzeSettings.Data settings = FtpBronzeSettings.getData();
        ...
        if (FtpBronzeSettings.getData().ignorePlaytime && Skills.getTotalLevel() >= 100 && Quests.getQuestPoints() >= 10) {
...
//
    ...
    public String[] getPaintInfo() {
        ...
        return new String[]{
                "cCBronzeFarm: " + runtime.formatTime(),
    ...
    public void onMouseClicked(MouseEvent e) {
        ...
        if (Gui.wasButtonClicked(e.getPoint())) {
            SwingUtilities.invokeLater(() -> new Gui(FtpBronzeSettings.getData()));
        }
    }
}
