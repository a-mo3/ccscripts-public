...
import org.dreambot.scriptdata.FtpClaySettings;
...
public class FtpClay extends PseudoScript implements ItemContainerListener {
    ...
    @Override
    public void init() {
        SettingsLoader.changePath("cCClayFarm");
        ...
        tree.addChildren(
                ...
                new MuleOff(FtpClaySettings.getData().hoursUntilMuleOff, FtpClaySettings.getData().moneyLeftAfterMuling)
                        .setSimpleName("Mule Off"),
                new GenericMineLeaf(() -> true, "Clay rocks", clayArea)
                        .setSimpleName("Mine clay")
                // todo add a mode for wet clay
        );
//        new AIAntiban();
    }
    ...
    @Override
    public int onLoop() {
        FtpClaySettings.Data settings = FtpClaySettings.getData();
        ...
        if (FtpClaySettings.getData().ignorePlaytime && Skills.getTotalLevel() >= 100 && Quests.getQuestPoints() >= 10) {
...
//
    ...
    public void onMouseClicked(MouseEvent e) {
        ...
        if (Gui.wasButtonClicked(e.getPoint())) {
            SwingUtilities.invokeLater(() -> new Gui(FtpClaySettings.getData()));
        }
    }
}
