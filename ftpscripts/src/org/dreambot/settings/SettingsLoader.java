...
public class SettingsLoader<SettingsClass> {
    public static String scriptPath = System.getProperty("scripts.path") + "/cCPvm/";
    ...
    public static void changePath(String scriptName) {
        ...
        Logger.info("Changed path " + scriptName);
    }
    ...
    public SettingsClass loadFile(String fileName, SettingsClass defaultSave) {
        ...
        Logger.info(scriptPath + " " + fileName);
        if (!f.exists()) {
    ...
    public void saveFile(String fileName, SettingsClass data) {
        ...
        try {
            Logger.info(String.format("Saved file: %s%s", scriptPath, fileName));
