...
public class MuleRequestEvent extends AbstractEvent implements ChatListener {
    ...
    String status = "";
    public static boolean moneyMakeMode = false;
    ...
    @Override
    public int onLoop() {
        log("money make " + moneyMakeMode);
        offeredItems.forEach(x -> log("Offering " + x.getItemId()));
        requiredItems.forEach(x -> log("Requiring " + x.getItemId()));
        if (moneyMakeMode && offeredItems.isEmpty() && requiredItems.stream().anyMatch(x -> x.getItemId() == ItemID.COINS_995)) {
            log("Money make mode is on, not finishing this request and going to get money instead");
            log("Make money " + GetMoneyBranch.gpTarget);
            GetMoneyBranch.gpTarget = (int) (requiredItems.stream()
                    .filter(x -> x.getItemId() == ItemID.COINS_995)
                    .findFirst().orElse(new RequiredItem(ItemID.COINS_995, 35_000)).getQuantity() * 1.2);
            setComplete(true);
        ...
        if (!loadout.isFulfilled() && !Trade.isOpen()) {
            log("Fulfilling loadout " + new WithdrawLoadoutEvent(loadout, null)
