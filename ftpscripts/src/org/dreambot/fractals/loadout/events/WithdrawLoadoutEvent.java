...
//                .forEach(x -> Logger.info(String.format("%s -%d * %d", x.getName(), x.getID(), x.getAmount())));
    ...
    public int onLoop() {
        ...
        if (equipmentLoadout != null && !equipmentLoadout.isFulfilled()) {
            ...
            LoadoutMapEntry i = equipmentLoadout.getLoadoutList().stream().filter(x -> x.getSlot() == EquipmentSlot.ARROWS || x.getSlot() == EquipmentSlot.WEAPON)
                    ...
                    .findFirst()
                    .orElse(null);
            if (i != null && Equipment.count(i.getItem().getItemId()) > i.getItem().getMax()) {
                Logger.info("Too much ammo - bank everything");
                new BankAllEquipmentEvent().execute();
            }
