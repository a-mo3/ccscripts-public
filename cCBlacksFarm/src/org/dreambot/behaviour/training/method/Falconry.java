...
public class Falconry extends Fractal {
    ...
    public Falconry(Supplier<Boolean> acceptCondition) {
        ...
        this.inventoryLoadout = new InventoryLoadout()
                ...
                .addItem(ItemID.COINS_995, 100, 10000)
        ;
    }
    ...
    public int onLoop() {
        if (Dialogues.inDialogue()) {
            Dialog.solve("have a go with your", "that seems", "Yes, please.");
            return ReactionGenerator.getNormal();
        }

        if (!Equipment.contains(ItemID.FALCONERS_GLOVE) && !Equipment.contains(ItemID.FALCONERS_GLOVE_10024)) {
            if (!Equipment.isSlotEmpty(EquipmentSlot.HANDS) || !Equipment.isSlotEmpty(EquipmentSlot.SHIELD) || !Equipment.isSlotEmpty(EquipmentSlot.WEAPON)) {
