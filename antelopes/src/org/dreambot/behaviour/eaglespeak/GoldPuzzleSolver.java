...
public class GoldPuzzleSolver extends Fractal {
    ...
    @Override
    public int onLoop() {
        if (PlayerSettings.getBitValue(PUZZLE_STATE_VARBIT) == 132) {
            ...
            if (Client.isDynamicRegion()) {
                log("Exit instance");
                GameObject t = GameObjects.closest("Tunnel");
                if (t != null) {
                    t.interact("Enter");
            ...
            GameObject resetLever = GameObjects.closest("Reset lever");
            if (resetLever != null) {
                log("pull reset lever");
                resetLever.interact("Pull");
                Sleep.sleepUntil(() -> PlayerSettings.getBitValue(PUZZLE_STATE_VARBIT) != 132, 4400);
                return ReactionGenerator.getNormal();
            }

            log("go to puzzle entrance");
