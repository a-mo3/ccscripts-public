...
import org.dreambot.timing.ReactionGenerator;
...
public class RedeemBondEvent extends AbstractEvent {
    ...
    public int onLoop() {
        ...
        if (Client.getMembershipLeft() > 0 || PlayerSettings.getConfig(1780) > 0) {
            Log.info("redeem bond event, you have membership");
            setComplete(true);
            return ReactionGenerator.getNormal();
        }
        ...
        if (parent != null && parent.isVisible()) {
            ...
            WidgetChild confirmButton = Widgets.get(x -> x.getParentID() == 66 && x.hasAction("Confirm"));
