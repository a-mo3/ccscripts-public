...
public class InventoryLoadout {
    ...
    public InventoryLoadout setEnabledConition(Supplier<Boolean> enabledConition) {
        lastAddedItem.setEnabledConition(enabledConition);
        return this;
    }
    ...
    public boolean isFulfilled() {
        for (InventoryLoadoutItem item : loadoutItems) {
            if (item.getEnabledConition() != null && !item.getEnabledConition().get()) {
                continue;
            }
            ...
            if (count < item.getMin() || count > item.getMax()) {
                Logger.info(String.format("Item above or below requirements id: %d min: %d, max: %d",
                        item.getItemId(), item.getMin(), item.getMax()));
                return false;
            }
        }
    ...
    public Item getStrictItem() {
        for (Item item : Inventory.all()) {
            ...
            if (loadoutItems.stream().noneMatch(loadoutItem -> {
                ...
                return loadoutItem.itemId == item.getID();
            })) {
                return item;
            }
        }
