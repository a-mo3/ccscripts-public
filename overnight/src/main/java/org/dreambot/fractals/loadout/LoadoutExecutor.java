...
public class LoadoutExecutor {
    ...
    public static int execInvLoadout(InventoryLoadout loadout) {
        ...
        if (Bank.count(missingItem.getItemId()) + invCount < missingItem.getMax()) {
            ...
            return buyItem(missingItem, loadout.getSellItems());
        }
    ...
    public static int buyItem(LoadoutItem missingItem, int... sellItems) {
        ...
        if (GrandExchange.contains(variant != null ? variant.getBaseId() : missingItem.getItemId())) {
            ...
            if (!Sleep.sleepUntil(GrandExchange::isReadyToCollect, 64000)) {
        ...
        int ownedGP = OwnedItems.count(ItemID.COINS_995);
        Logger.format("Trying to buy %s\ntotal cost: %d\nownedGp: %d", new Item(missingItem.getItemId(), 0).getName(), price, ownedGP);
        if (ownedGP < price) {
            if (OwnedItems.containsAny(sellItems)) {
                new SellAllEvent(sellItems).execute();
                return ReactionGenerator.getNormal();
            }
            Logger.format("Does not have enough gp for this loadout, requesting %d from mule", missingItem.getMuleRequestAmount());
            new MuleRequestEvent()
                    .addRequiredItem(ItemID.COINS_995, price)
                    .execute();
            return ReactionGenerator.getNormal();
        }
