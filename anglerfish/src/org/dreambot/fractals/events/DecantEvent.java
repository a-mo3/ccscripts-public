...
public class DecantEvent extends AbstractResponseEvent<DecantEvent.Response> {
    ...
    private InventoryLoadout getInventoryLoadout() {
        return new InventoryLoadout()
                .addItem(getNotedId(PRAYER_POTION1), OwnedItems.count(PRAYER_POTION1, true))
                .addItem(getNotedId(PRAYER_POTION2), OwnedItems.count(PRAYER_POTION2, true))
                .addItem(getNotedId(PRAYER_POTION3), OwnedItems.count(PRAYER_POTION3, true))
                .addItem(getNotedId(STAMINA_POTION1), OwnedItems.count(STAMINA_POTION1, true))
                .addItem(getNotedId(STAMINA_POTION2), OwnedItems.count(STAMINA_POTION2, true))
                .addItem(getNotedId(STAMINA_POTION3), OwnedItems.count(STAMINA_POTION3, true));
    ...
    public int onLoop() {
        ...
        if (!getInventoryLoadout().isFulfilled()) {
            Logger.info("Withdraw potions: " + new WithdrawLoadoutEvent(getInventoryLoadout(),
                    null)
                    .setBuyRemainder(false)
                    .executed());
            return ReactionGenerator.getNormal();
        }
