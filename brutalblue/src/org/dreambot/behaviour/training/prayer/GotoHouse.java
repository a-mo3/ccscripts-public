...
public class GotoHouse extends Fractal {
    ...
    private WidgetChild getBestEnter() {
        ...
        for (WidgetChild button : enterButtons) {
            int houseIndex = button.getID();
//            Log.info("House index - " + houseIndex);
            WidgetChild nameWidget = Widgets.get(ADVERT_PARENT, NAME_PARENT, houseIndex);
            if (nameWidget == null) {
                Log.info("couldnt find name widget so button was skipped");
                continue;
            }

            String houseOwner = nameWidget.getText();
            if (blacklistedOwners.contains(houseOwner) || houseOwner.equals("Pyramid") || houseOwner.equals("V 3") || houseOwner.equals("XVLKLEOPATRA")) {
                Log.info("this owner was blacklisted");
                continue;
            }
