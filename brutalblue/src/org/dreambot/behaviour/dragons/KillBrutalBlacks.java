...
public class KillBrutalBlacks extends Fractal implements ItemContainerListener {
    Tile safeSpot = new Tile(1613, 10086);

    public KillBrutalBlacks(Supplier<Boolean> acceptCondition) {
        super(acceptCondition);
        Client.getInstance().addEventListener(this);
    }

    final Area BLACK_DRAGON_AREA = new Area(1608, 10105, 1622, 10088);
    final Area ALL_DRAGONS = new Area(1607, 10108, 1636, 10069);
    //    public static final Timer lastDragonAtk = new Timer(500);
    boolean getBlackAgro = false;
    ...
    public int onLoop() {
        ...
        if (interacting != null && interacting.getName().toLowerCase().contains("black")) {
            getBlackAgro = false;
        }

//        if (getBlackAgro ||
//                (interacting != null && interacting.getName().toLowerCase().contains("black"))) {
//            Logger.info("Safespot");
//            getBlackAgro = true;
//            if (Walking.shouldWalk()) Walking.walk(BLACK_DRAGON_AREA);
//            return ReactionGenerator.getQuick();
//        }
        ...
        // todo high alch
        if (GroundItems.closest(x -> BLACK_DRAGON_AREA.contains(x) && x.getItem().getLivePrice() > 300) != null) {
            Logger.info("Loot event: " + new SmartLootEvent(() -> GroundItems.all(x -> BLACK_DRAGON_AREA.contains(x) && x.getItem().getLivePrice() > 300),
        ...
        NPC dragon = NPCs.closest(x -> x.getName().equals("Brutal black dragon") && !x.isInCombat());
    ...
    public void onInventoryItemAdded(Item item) {
        if (BLACK_DRAGON_AREA.contains(Players.getLocal())) {
