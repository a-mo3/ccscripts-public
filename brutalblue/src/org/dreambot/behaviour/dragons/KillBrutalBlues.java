...
public class KillBrutalBlues extends Fractal implements ItemContainerListener {
    Tile safeSpot = new Tile(1626, 10073);

    public KillBrutalBlues(Supplier<Boolean> acceptCondition) {
        super(acceptCondition);
        Client.getInstance().addEventListener(this);
    }

    final Area BLUE_DRAGON_AREA = new Area(1626, 10086, 1637, 10067);
    final Area ALL_DRAGONS = new Area(1607, 10108, 1636, 10069);
    //    public static final Timer lastDragonAtk = new Timer(500);
    boolean getBlueAgro = false;
    ...
    public int onLoop() {
        ...
        if (ScriptSettings.getSettingsData().dragonMode == DragonMode.BLUE
                && interacting != null
                && interacting.getName().toLowerCase().contains("blue")) {
            getBlueAgro = false;
        }

        if (getBlueAgro || (ScriptSettings.getSettingsData().dragonMode == DragonMode.BLUE && interacting != null
                && interacting.getName().toLowerCase().contains("red")) && interacting.getName().contains("dragon")) {
            getBlueAgro = true;
            Logger.info("Getting blue dragon agro");
            if (Walking.shouldWalk()) Walking.walk(BLUE_DRAGON_AREA);
            return ReactionGenerator.getQuick();
        }

        if (!safeSpot.equals(Players.getLocal().getTile())) {
            Logger.info("Walking to safespot");
        ...
        if (GroundItems.closest(x -> BLUE_DRAGON_AREA.contains(x) && x.getItem().getLivePrice() > 300) != null) {
            Logger.info("Loot event: " + new SmartLootEvent(() -> GroundItems.all(x -> BLUE_DRAGON_AREA.contains(x) && x.getItem().getLivePrice() > 300),
        ...
        NPC dragon = NPCs.closest(x -> x.getName().equals("Brutal blue dragon") && !x.isInCombat());
    ...
    public void onInventoryItemAdded(Item item) {
        if (BLUE_DRAGON_AREA.contains(Players.getLocal())) {
