...
public class BuyItemsEvent extends AbstractResponseEvent<BuyItemsEvent.Response> {
    ...
    public int onLoop() {
        ...
        timeStampMap.forEach((x, y) -> Logger.info(String.format("%d offered %d ago, ownedCount: %d, in ge: %b",
                x, System.currentTimeMillis() - y, OwnedItems.count(x), GrandExchange.contains(x))));
...
//                Sleep.sleepUntil(() -> !GrandExchange.contains(rebid.getBaseID()), 2400);
        ...
        for (LoadoutItem bidItem : itemsToBidOn) {
            ...
            if (!GrandExchange.contains(bidItem.getBaseID())) {
                int price = Math.max(bidItem.getBuyPrice() + 2, 2 + priceMap.getOrDefault(bidItem.getBaseID(), 0)) * bidItem.getRefill();
                ...
                GrandExchange.buyItem(bidItem.getBaseID(), bidItem.getRefill(), 2 + priceMap.get(bidItem.getBaseID()));
