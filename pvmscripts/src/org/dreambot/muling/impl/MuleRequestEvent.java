...
import org.dreambot.behaviour.misc.MuleOffSettings;
...
public class MuleRequestEvent extends AbstractEvent implements ChatListener {
    ...
    boolean disableMuling = false;
    MuleOffSettings settings;

    private final String identifier;

    public MuleRequestEvent(String identifier) {
        this.identifier = identifier;
        settings = SettingsRepository.getSetting("Muling", new MuleOffSettings());
        if (settings != null && settings.disableMuleOn) disableMuling = true;
    ...
    public int onLoop() {
        ...
        if (disableMuling && offeredItems.isEmpty()) {
            log("Mule on is disabled, turning off script");
        ...
        if (timeoutMuleRequest.isExpired()) {
            if (Worlds.getCurrentWorld() != startWorld) {
                ...
                Antiban.sleepUntil(() -> Worlds.getCurrentWorld() == startWorld, 5000);
                return ReactionGenerator.getNormal();
            }
            setComplete(true);
        ...
        if (!loadout.isFulfilled() && !Trade.isOpen()) {
            WithdrawLoadoutEvent.Response  p = new WithdrawLoadoutEvent(loadout, null)
                    .setMuleRequiredGP(false).executed();
            log("Fulfilling loadout " + p);
            if (p == WithdrawLoadoutEvent.Response.NOT_ENOUGH_GP) {
                setComplete(true);
                setFailed(true);
                log("Halting mule event wrong gp");
                return ReactionGenerator.getNormal();
            }
        ...
        if (Trade.isOpen(1)) {
            ...
            if (!timeoutMuleRequest.requestSideHasFulfilled()) {
                ...
                if (offeredItem == null) {
                    ...
                    Trade.acceptTrade();
                    Antiban.sleepUntil(() -> Trade.hasAcceptedTrade(TradeUser.US), 1000);
                    return ReactionGenerator.getNormal();
                }
                log("%d * %d", offeredItem.getItemId(), offeredItem.getQuantity());
                Trade.addItem(offeredItem.getItemId(), offeredItem.getQuantity());
                Antiban.sleepUntil(() -> Trade.contains(true, offeredItem.getItemId()), 2400);
                return ReactionGenerator.getNormal();
            }
        ...
        if (muleResponse.world == 0) {
            ...
            Alerts.addAlert(3500, Color.RED, "Connected but no mule replied, make sure mule has enough gp");
            Sleep.sleep(3000);
            setFailed(true);
            return ReactionGenerator.getNormal();
        }
        ...
        if (mulePlayer != null) {
            ...
            Antiban.sleepUntil(Trade::isOpen, 4400);
            return ReactionGenerator.getNormal();
        }
    ...
    public MuleRequestEvent addRequiredItem(int itemID, int quantity) {
        if (itemID == ItemID.COINS_995 && quantity < MuleOff.minMuleOff) quantity = MuleOff.minMuleOff;
    ...
    private MuleConnection getMuleConnection() {
        ...
        if (muleConnection == null) {
            Logger.info("Getting mule connection " + MuleState.MULE_SERVER_ADDRESS);
            try {
    ...
    private MuleResponseMessage sendMuleRequest() {
        ...
        try {
            log("Sending mule message " + jsonMuleRequest);
