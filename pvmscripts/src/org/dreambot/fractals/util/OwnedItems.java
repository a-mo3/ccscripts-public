...
public class OwnedItems {
    public static List<Item> all() {
        List<Item> all = Inventory.all();
        all.addAll(Bank.all());
        return all;
    }

    public static List<Item> all(Filter<Item> filter) {
        List<Item> all = Inventory.all(filter);
        all.addAll(Bank.all(filter));
        return all;
    }
    ...
    public static int count(Filter<Item> itemFilter) {
        return Inventory.count(itemFilter) + Bank.count(itemFilter);
    }
    ...
    public static boolean contains(String name) {
        return Inventory.contains(name) || Bank.contains(name) || Equipment.contains(name);
    }
    ...
    public static boolean contains(Filter<Item> filter) {
        return Equipment.contains(filter) || Bank.contains(filter) || Inventory.contains(filter);
    }
    ...
    public static boolean containsAny(Integer[] ids) {
        for (int id : ids) {
            if (contains(id)) return true;
        }
        return false;
    ...
    public static boolean containsAnyUnworn(int... ids) {
        for (int id : ids) {
            if (containsUnworn(id)) return true;
        }
        return false;
    }
