...
//        this.strictIgnoredItemIDs = inventoryLoadout.strictIgnoredItemIDs;
    ...
    /**
     * delete inventory items matching a condition, useful for when you are cloning a premade loadout
     *
     * @param removeCondition
     * @return
     */
    public InventoryLoadout removeItems(Predicate<InventoryLoadoutItem> removeCondition) {
        loadoutItems.removeIf(removeCondition);
        return this;
    }
    ...
    public InventoryLoadout setBuyPrice(int buyPrice) {
        ...
        lastAddedItem.setMaxPrice(buyPrice);
        return this;
    }
    ...
    public InventoryLoadout setRefill(Supplier<Integer> refillSupplier) {
        lastAddedItem.setRefillSupplier(refillSupplier);
        return this;
    }
    ...
    public InventoryLoadout addItem(Supplier<Integer> idSupplier, Supplier<Integer> minSupp) {
        InventoryLoadoutItem item = new InventoryLoadoutItem(idSupplier, minSupp);
        lastAddedItem = item;
        loadoutItems.add(item);
        return this;
    }

    public InventoryLoadout addItem(int itemid, Supplier<Integer> minSupplier) {
        InventoryLoadoutItem item = new InventoryLoadoutItem(itemid, minSupplier);
        lastAddedItem = item;
        loadoutItems.add(item);
        return this;
    }
    ...
    public boolean isFulfilled() {
        for (InventoryLoadoutItem item : loadoutItems) {
            if (!Client.isMembers() && new Item(item.getItemId(), 1).isMembersOnly()) {
                Logger.info("skip item thats mems only " + item.itemId);
                continue;
            }
    ...
    public Item getStrictItem() {
        for (Item item : Inventory.all()) {
            ...
            if (loadoutItems.stream()
                    // todo this is probably how loadouts should work, but everything is written in a way where this breaks stuff
//                    .filter(x -> x.getEnabledCondition() == null || x.getEnabledCondition().get())
                    .noneMatch(loadoutItem -> {
    ...
    public InventoryLoadout enabledIfOwned() {
    ...
    public InventoryLoadout clone() {
        return new InventoryLoadout(this);
    }
}
