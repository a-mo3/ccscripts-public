...
public class GetMembership extends Fractal implements ConfigurableFractal<GetMembershipSettings> {
    ...
    public int onLoop() {
        ...
        GetMembershipSettings settings = getSettings();
        if (!OwnedItems.contains(BOND) && !GrandExchange.contains(ItemID.OLD_SCHOOL_BOND, ItemID.OLD_SCHOOL_BOND_UNTRADEABLE)
                && OwnedItems.count(ItemID.COINS_995) < settings.initalGP) {
            log("Requesting GP from mule for bond");
            new MuleRequestEvent(ScriptManager.getScriptManager().getCurrentScript().getSDNName())
                    .addRequiredItem(ItemID.COINS_995, settings.initalGP - OwnedItems.count(ItemID.COINS_995))
                    .setInterruptCondition(Client::isMembers)
                    .execute();
            return ReactionGenerator.getNormal();
        }

        new RedeemBondEvent(getSettings().bondPrice)
                .execute();
        return ReactionGenerator.getNormal();
    }
