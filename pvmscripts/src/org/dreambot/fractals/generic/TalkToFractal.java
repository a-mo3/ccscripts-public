...
public class TalkToFractal extends Fractal {
    ...
    public int onLoop() {
        ...
        if (Dialogues.inDialogue()) {
            ...
            if (afterChat != null) Antiban.sleepUntil(afterChat, sleepTimeout);
        ...
        if (area != null && !area.contains(Players.getLocal())) {
            if (targetTile == null) {
                ...
                if (Walking.shouldWalk()) Walking.walk(targetTile);
            }
            return ReactionGenerator.getNormal();
        }
        ...
        if (doReachCheck) {
            if (npc != null && !npc.canReach()) {
                if (Walking.shouldWalk(6)) Walking.walk(npc.getTile());
                return ReactionGenerator.getNormal();
            }
        }

        if (npc != null && npc.interact(x -> Arrays.stream(interaction).anyMatch(i -> i.contains(x)))) {
            Antiban.sleepUntil(Dialogues::inDialogue, sleepTimeout);
        }
        return ReactionGenerator.getNormal();
    }
}
