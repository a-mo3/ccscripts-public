...
import org.dreambot.behaviour.method.lavadragons.ExitLavaDragons;
import org.dreambot.behaviour.method.lavadragons.KillLavaDragons;
import org.dreambot.behaviour.method.lavadragons.LavaDragonConst;
import org.dreambot.behaviour.method.lavadragons.LavaDragonLocation;
...
import static org.dreambot.behaviour.method.spindel.RechargeWildyWeapon.needsToRecharge;

public class LavaDragonScript extends PseudoScript implements ItemContainerListener, ChatListener {
    FractalRoot<LavaDragonSettings> tree = new FractalRoot<>(new LavaDragonSettings(), getScriptName());

    @Override
    public void init() {
        Client.getInstance().addEventListener(this);
        tree.setSimpleName("cCLavaDragonFarm");

        // switches world before logging in always so anti pk doesnt need to do that
        ...
        LavaDragonConst.location = LavaDragonLocation.values()[Calculations.random(0, LavaDragonLocation.values().length)];
        LavaDragonSettings settings = tree.getSettings();
        LavaDragonConst.settings = settings;
        tree.addChildren(
                ...
                new F2PMagicBranch(() -> Skills.getRealLevel(Skill.MAGIC) < settings.ftpMagicTarget, settings.ftpDefenseTarget)
                ...
                new AntiPkBranch().setSimpleName("Anti pk branch"),
                ...
                new RechargeWildyWeapon(ItemID.ACCURSED_SCEPTRE_U, ItemID.ACCURSED_SCEPTRE, rechargedExitDanger,
                        250).setSimpleName("Recharge Accursed")
                        .setAcceptCondition(() -> tree.getSettings().lavaDragonLoadout.name().contains("ACCU")
                                && (OwnedItems.contains(ItemID.ACCURSED_SCEPTRE_U) || needsToRecharge)),
                new MuleOff().setSimpleName("Mule off"),

                new ExitLavaDragons(tree.getSettings()),
                new KillLavaDragons(() -> true, settings)
                        .setSimpleName("Kill Lava dragons")
        );
    }
