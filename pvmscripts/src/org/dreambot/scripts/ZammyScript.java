...
public class ZammyScript extends PseudoScript implements ItemContainerListener, SpawnListener {
    FractalRoot<ZammySettings> tree = new FractalRoot<>(new ZammySettings(), getScriptName());
...
//            .setRefill(5_000)
    ...
    public void init() {
        ...
        tree.setSimpleName("cCZamorak");

        TickRangeZammyBranch bossFight = new TickRangeZammyBranch(() -> KillZammy.ZAMMY_ARENA.contains(Players.getLocal())
                ...
                tree.getSettings());
        bossFight.setSimpleName("Kill Zam");

        tree.addChildren(
                new EmptyDeathsCoffer().setSimpleName("Death coffer"),
                new Fractal(() -> KillZammy.ZAMMY_ARENA.contains(Players.getLocal()))
                ...
                SandCrabs.getRange(() -> Skills.getRealLevel(Skill.DEFENCE) < Math.max(60, tree.getSettings().defenceTarget)
                                || Skills.getRealLevel(Skill.RANGED) < tree.getSettings().rangeTarget)
                        .setDefenceTarget(Math.max(60, tree.getSettings().defenceTarget))
...
//                new MakeScorchingBow(() -> Bank.isCached() && !OwnedItems.contains(ItemID.SCORCHING_BOW))
//                        .setSimpleName("Scorching bow"),
                bossFight,
                new GetZammyKC(() -> true, tree.getSettings())
...
//        new AIAntiban();
    ...
    public void onNpcDespawn(NPC npc) {
        ...
        if (npc.getName().equals("K'ril Tsutsaroth")) {
    ...
    public void onScriptPaint(Graphics g) {
        ...
        if (zam != null) {
//            for (Tile tile : zam.getArea().getTiles()) {
//                g.drawPolygon(tile.getPolygon());
//            }
//            g.setColor(Color.blue);
//            g.drawPolygon(zam.getTrueTile().getPolygon());

            g.setColor(Color.ORANGE);
            g.drawPolygon(zam.getServerTile().getPolygon());
        }
