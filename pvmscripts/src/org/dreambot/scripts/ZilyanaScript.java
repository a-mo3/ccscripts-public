...
import org.dreambot.behaviour.method.gwd.zilyana.ZilyanaTile;
...
public class ZilyanaScript extends PseudoScript implements ItemContainerListener, SpawnListener {
    FractalRoot<ZilyanaSettings> tree = new FractalRoot<>(new ZilyanaSettings(), getScriptName());
...
//                ItemID.TARROMIN_POTION_UNF
        ...
        BankLocation.blacklist(BankLocation.TZHAAR);
        BankLocation.blacklist(BankLocation.TZHAAR_EAST);
        BankLocation.blacklist(BankLocation.TAL_TEKLAN);
        ...
        tree.setSimpleName("cCZilyanas");

        TickKillZilyanaBranch bossFight = new TickKillZilyanaBranch(() -> KillZilyana.ZILYANA_BOSS_ROOM.contains(Players.getLocal())
                || (GetZilyanaKC.getSaradominKC() >= 40 && Players.getLocal().getY() > 4000)
                // < 8000 y is a check for not in the wilderness gwd
                || (Inventory.contains(ItemID.ECUMENICAL_KEY) && Players.getLocal().getY() > 4000 && Players.getLocal().getY() < 8000), tree.getSettings());
        bossFight.setSimpleName("Kill zil");

        tree.addChildren(
                new EmptyDeathsCoffer().setSimpleName("Death coffer"),
                new Fractal(() -> KillZilyana.ZILYANA_BOSS_ROOM.contains(Players.getLocal()))
                ...
                new DistributedRangeTraining(() -> Skills.getRealLevel(Skill.RANGED) < tree.getSettings().f2pRangeTarget)
                ...
                new TimedShuffleFractal(Calculations.random(40, 125))
                        .addChildren(
                                new ConfigurableMagicBranch(() -> Skills.getRealLevel(Skill.MAGIC) < Calculations.random(61))
                                        .setSimpleName("rand Magic for TP & Alch"),
                                new ConfigurableMagicBranch(() -> Skills.getRealLevel(Skill.MAGIC) < 61)
                                        .setSimpleName("61 Magic for TP & Alch"),
                                new ConfigurableMeleeTraining(() -> (tree.getSettings().ecuKeyStrategy == EcuKeyStrategy.AVIANSIES && Skills.getRealLevel(Skill.STRENGTH) < 60)).setSimpleName("60 Str"),
                                new AgilityBranch(() -> Skills.getRealLevel(Skill.AGILITY) < 20).setSimpleName("20 Agil"),
                                new AgilityBranch(() -> Skills.getRealLevel(Skill.AGILITY) < 50).setSimpleName("20 Agil"),
                                new AgilityBranch(() -> Skills.getRealLevel(Skill.AGILITY) < 70).setSimpleName("70 Agil"),

                                new PrayerBranch(() -> Skills.getRealLevel(Skill.PRAYER) < Math.max(20, tree.getSettings().prayerTarget))
                                        .setSimpleName("Prayer (20)"),
                                new PrayerBranch(() -> Skills.getRealLevel(Skill.PRAYER) < Math.max(43, tree.getSettings().prayerTarget))
                                        .setSimpleName("Prayer (43 min)"),
                                new ConfigurableRangeTraining(() -> Skills.getRealLevel(Skill.RANGED) < 35)
                ...
                new ConfigurableRangeTraining(() -> Skills.getRealLevel(Skill.DEFENCE) < Math.max(60, tree.getSettings().defenceTarget)
                        || Skills.getRealLevel(Skill.RANGED) < tree.getSettings().rangeTarget, Math.max(60, tree.getSettings().defenceTarget))
                        .setLoadoutCondition(() -> Players.getLocal().getY() < 3500) // ive had reports of this fractal randomly activating
                        .setSimpleName("Range training (60 def minimum)"),
                ...
                new Fractal(() -> (Players.getLocal().getY() < 4000 || Players.getLocal().getY() > 6000)
                        && tree.getSettings().ecuKeyStrategy == EcuKeyStrategy.AVIANSIES
                        && (!OwnedItems.contains(ItemID.ECUMENICAL_KEY) || (!keyLock && tree.getSettings().threeKeysPerTrip)))
                        .addChildren(
...
//                new MakeTrollHeimTabs(),
                ...
                bossFight,
                new ZilyanaKCTickBranch(() -> true, tree.getSettings())
...
//        new AIAntiban();
    ...
    public int onLoop() {
        ...
        if (Client.getGameStateId() == 10) return ReactionGenerator.getNormal();
        if (!Client.isLoggedIn()) {
            Logger.info("Not logged in return");
    ...
    public String[] getPaintInfo() {
        ...
        return new String[]{
                ...
                "target: " + target,
                "Zil GP (Actual) " + grossGp,
                "Zil GP (EV) " + zilyanaDeathCounter * 220_000,
                "Zil kills " + zilyanaDeathCounter,
    ...
    public String getScriptName() {
        return "cCZilyanaFarm";
    }

    @Override
    public int getMoneyMade() {
        return grossGp + KillZilyana.zilyanaGP;
    }
    ...
    public void onScriptPaint(Graphics g) {
        ...
        Tile lps = Players.getLocal().getServerTile();
        if (lps != null && lps.getPolygon() != null) {
            g.setColor(Color.GRAY);
            g.drawPolygon(lps.getPolygon());
        }

        ZilyanaTile.tileGraph.forEach(zt -> {
            Polygon ztp = zt.location.getPolygon();
            if (ztp != null) {
                g.setColor(zt.attackTile ? Color.RED : Color.YELLOW);
                g.drawPolygon(ztp);
            }
        });
    }
