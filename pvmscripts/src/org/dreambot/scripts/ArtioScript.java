...
import org.dreambot.behaviour.method.artio.*;
...
public class ArtioScript extends PseudoScript implements ItemContainerListener, ChatListener, GameStateListener {
    FractalRoot<ArtioSettings> tree = new FractalRoot<>(new ArtioSettings(), getScriptName());
    AtomicBoolean isBreaking = new AtomicBoolean(false);
    BooleanSupplier breakingSupplier = () -> isBreaking.get();
    int deathCount = 0;

//    private ArtioSettings tree.getSettings() {
//        return SettingsRepository.getSetting(getScriptName(), new ArtioSettings());
//    }
    ...
    public void onArgs(String... args) {
        for (String arg : args) {
            Arrays.stream(ArtioLoadout.values())
    ...
    public void init() {
        ...
        WithdrawLoadoutEvent.sellList = LootArtio.LOOT;
        MuleOff.LOOT = LootArtio.LOOT;

        Logger.info("Init");
        tree.setSimpleName("cCArtio");
        ...
        tree.addChildren(
                ...
                new ConfigurableMeleeTraining(() -> {
                    ArtioSettings settings = tree.getSettings();
                ...
                new ConfigurableMagicBranch(() -> Skills.getRealLevel(Skill.MAGIC) < Math.max(79, tree.getSettings().magicTarget))
...
//                SandCrabs.getRange(() -> true)
                ...
                new PrayerBranch(() -> Skills.getRealLevel(Skill.PRAYER) < tree.getSettings().prayerTarget).setSimpleName("Prayer Training"),
                ...
                new RechargeWildyWeapon(ItemID.ACCURSED_SCEPTRE_U, ItemID.ACCURSED_SCEPTRE, rechargedExitDanger,
                        ...
                        .setAcceptCondition(() -> tree.getSettings().loadout.name().contains("ACCURSED")
                ...
                new RechargeWildyWeapon(ItemID.CRAWS_BOW_U, ItemID.CRAWS_BOW, rechargedExitDanger,
                        ...
                        .setAcceptCondition(() -> tree.getSettings().loadout.name().contains("CRAW")
                ...
                new RechargeWildyWeapon(ItemID.WEBWEAVER_BOW_U, ItemID.WEBWEAVER_BOW, rechargedExitDanger,
                        ...
                        .setAcceptCondition(() -> tree.getSettings().loadout.name().contains("WEBWEAVER")
                ...
                new ExitArtioWithLoot(tree.getSettings().exitLootValue, tree.getSettings().maxKillsPerRun)
                ...
                new ArtioBranch(() -> true).setSimpleName("Fight Artio")
        );
//        new AIAntiban();
...
//        if (GoToArtio.Artio_ARENA.contains(Players.getLocal())) hasLootInBag = true;
    ...
    public void onScriptPaint(Graphics g) {
        ...
        if (artio != null) {
            ...
            safeTiles = Arrays.stream(artio.getSurroundingArea(5).getTiles())
                    .filter(Tile::canReach)
                    // todo optimize this retarded filter
                    .filter(t -> t.getY() < artio.getY())
                    .filter(tile -> GameObjects.closest(x -> x.getId() == MagicFightArtio.BEAR_TRAP_ID && tile.equals(x.getTile())) == null)
        ...
        if (!safeTiles.isEmpty()) {
            Tile t = safeTiles.stream()
                    .min(Comparator.comparingDouble(Tile::distance))
                    .orElse(null);
            g.setColor(Color.GREEN);
            if (t != null) g.drawPolygon(t.getPolygon());
        }
    }
    ...
    public void onInventoryItemAdded(Item item) {
        ...
        if (!GoToArtio.ARTIO_ARENA.contains(Players.getLocal())) return;
