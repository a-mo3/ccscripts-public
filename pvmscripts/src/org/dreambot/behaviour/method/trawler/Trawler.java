package org.dreambot.behaviour.method.trawler;
...
public class Trawler extends Fractal implements ChatListener {
    ...
    final int MAX_CONTRIBUTION = 750;
    final Timer bankCheckTime = new Timer(60L * 1000 * 5);
    ...
    public Trawler() {
        ...
        this.inventoryLoadout = new InventoryLoadout()
                .addItem(ItemID.SWAMP_PASTE, 100, 2000)
    ...
    public int onLoop() {
        ...
        if (getContribution() >= MAX_CONTRIBUTION) {
            log("At max contribution " + MAX_CONTRIBUTION);
            return ReactionGenerator.getLong();
        }
        // walk to edge
        if (lp.getY() == 4825) {
        ...
        GameObject leak = GameObjects.closest(x -> x.getName().equals("Leak") && x.distance() < 3 && x.hasAction("Fill"));
        if (leak != null) {
            log("Fill leak");
            leak.interact("Fill");
        }
