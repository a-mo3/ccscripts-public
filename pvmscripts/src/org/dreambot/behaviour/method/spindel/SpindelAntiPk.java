...
public class SpindelAntiPk extends Fractal implements HitSplatListener {
    ...
    int lastTellyBlockWorld = -1;
    Timer tellyBlockTimer = new Timer(60 * 1000 * 5);
    ...
    public static int leaveSpindel() {
        Logger.info("Leave spindel");
        ...
        if (SPINDEL_CHASM.contains(Players.getLocal()) && Players.getLocal().isInCombat()) {
            ...
            if (!SPINDEL_EXIT_CAVE.contains(Players.getLocal())) {
                Walking.walk(SPINDEL_EXIT_CAVE);
        ...
        if (Equipment.contains(ItemVariants.COMBAT_BRACLET.getIds())) {
            ...
            Equipment.interact(EquipmentSlot.HANDS, "Edgeville");
            Sleep.sleepUntil(() -> !Combat.isInWild(), 1400);
        }
        return ReactionGenerator.getQuick();
    }
    ...
    /**
     * dont tp out just walk out, used for finding a world
     *
     * @return
     */
    public static int walkOut() {
        UtilProvider.staminaUp();

        if (SPINDEL_CHASM.contains(Players.getLocal())) {
            // go to exit cave
            if (!SPINDEL_EXIT_CAVE.contains(Players.getLocal())) {
                Walking.walk(SPINDEL_EXIT_CAVE);
                return ReactionGenerator.getQuick();
