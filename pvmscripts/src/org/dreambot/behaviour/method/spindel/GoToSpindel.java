...
public class GoToSpindel extends Fractal implements ChatListener {
    static Area WEB_CHASM_ENTRANCE = new Area(3180, 3747, 3185, 3742);
    ...
    public GoToSpindel() {
        super(() -> !SPINDEL_CHASM.contains(Players.getLocal()));
        Client.getInstance().addEventListener(this);

        this.inventoryLoadout = SettingsRepository.findInstanceOf(new SpindelSettings()).loadout.loadout.setStrictSupplier(() -> !Combat.isInWild());
        this.equipmentLoadout = SettingsRepository.findInstanceOf(new SpindelSettings()).loadout.equipmentLoadout;
    }
    ...
    public int onLoop() {
        ...
        if (!WEB_CHASM_ENTRANCE.contains(Players.getLocal())) {
            UtilProvider.staminaUp();
            if (Walking.shouldWalk()) Walking.walk(WEB_CHASM_ENTRANCE);
            return ReactionGenerator.getQuick();
        }
...
//                    Sleep.sleepUntil(() -> Worlds.getCurrentWorld() == world.getWorld(), 6400);
        ...
        if (entrance != null) {
            entrance.interact("Enter");
            Sleep.sleepUntil(() -> CombatUtil.getThreat() != null || WEB_CHASM_ENTRANCE.contains(Players.getLocal()),
                    1200);
        }
