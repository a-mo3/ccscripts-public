...
import org.dreambot.scriptdata.SunlightAntelopeSettings;
...
public class SunlightAntelopes extends Fractal {
    public SunlightAntelopes(Supplier<Boolean> acceptCondition) {
        ...
        setSimpleName("Sunlight Antelopes");
        ...
        this.inventoryLoadout = new InventoryLoadout()
                ...
                .setRefill(100)
                .strictIgnore(ItemID.LOGS, ItemID.SUNLIGHT_ANTELOPE_ANTLER, ItemID.SUNLIGHT_ANTLER_BOLTS,
                        ItemID.SUNLIGHT_ANTELOPE_FUR, ItemID.RAW_SUNLIGHT_ANTELOPE)
    ...
    public int onLoop() {
        ...
        if (Inventory.contains(ItemID.JUG, ItemID.RAW_SUNLIGHT_ANTELOPE, ItemID.BIG_BONES, ItemID.SUNLIGHT_ANTELOPE_FUR)) {
            log("Drop all garbage");
            Inventory.dropAll(ItemID.JUG, ItemID.RAW_SUNLIGHT_ANTELOPE, ItemID.BIG_BONES, ItemID.SUNLIGHT_ANTELOPE_FUR);
            return ReactionGenerator.getNormal();
        }
        ...
        // fletch
        if (baitedAntelope == null && Inventory.contains(ItemID.SUNLIGHT_ANTELOPE_ANTLER)
                && SettingsRepository.findInstanceOf(new SunlightAntelopeSettings()).fletchBolts) {
            log("Fletching antlers to save space");
            if (ItemProcessing.isOpen()) {
                ItemProcessing.makeAll(ItemID.SUNLIGHT_ANTLER_BOLTS);
                Sleep.sleepUntil(() -> !Inventory.contains(ItemID.SUNLIGHT_ANTELOPE_ANTLER), 4400);
        ...
        // go to antelopes
        if (!SUNFIRE_AREA.contains(Players.getLocal())) {
        ...
        if (baitedAntelope == null && isTrapLayed && baitTimer.finished()) {
            NPC antelopeNpc = NPCs.closest(x -> x.getName().equals("Sunlight antelope") && x.getInteractingCharacter() == null, northPit.getTile());
        ...
        if (!Inventory.contains(ItemID.LOGS) && !isTrapLayed) {
            GameObject tree = GameObjects.closest(x -> x.getName().equals("Tree") && x.hasAction("Chop down"));
            log("Take logs " + tree);
            if (tree != null) {
