...
public class BlastFurnaceRune extends Fractal {
    public BlastFurnaceRune(Supplier<Boolean> acceptCondition) {
        super(acceptCondition);
        setSimpleName("Blast furnace Rune");
    }
    ...
    public int onLoop() {
        ...
        if (BarsOres.RUNITE_ORE.getValue() >= 5 && BarsOres.COAL.getValue() < 4) {
        ...
        if (Inventory.contains(ItemID.RUNITE_ORE)) {
            BlastFurnaceUtil.putOreOnBelt();
            return ReactionGenerator.getNormal();
        }
        ...
        // take bars out
        if (!Inventory.isFull() && BarsOres.RUNITE_BAR.getValue() > 0) {
            // take the bars out
        ...
        if (Bank.contains(ItemID.COAL) && Inventory.count(ItemID.COAL) < 13) {
            Bank.withdraw(ItemID.COAL, 13);
            Sleep.sleepUntil(() -> Inventory.count(ItemID.COAL) == 13, 2000);
            Bank.withdraw(ItemID.RUNITE_ORE, 12);
            return ReactionGenerator.getNormal();
        }
        return ReactionGenerator.getNormal();
    }
