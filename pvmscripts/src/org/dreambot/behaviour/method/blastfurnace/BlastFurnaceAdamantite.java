...
public class BlastFurnaceAdamantite extends Fractal {
    public BlastFurnaceAdamantite(Supplier<Boolean> acceptCondition) {
        super(acceptCondition);
        setSimpleName("Blast furnace Adamant");
    }
    ...
    public int onLoop() {
        ...
        if (BarsOres.ADAMANTITE_ORE.getValue() >= 9 && BarsOres.COAL.getValue() < 3) {
            log("Fixing excess ores");
            log("Coal count " + OwnedItems.count(ItemID.COAL));
        ...
        if (Inventory.contains(ItemID.ADAMANTITE_ORE)) {
            BlastFurnaceUtil.putOreOnBelt();
            return ReactionGenerator.getNormal();
        }

        // todo check you even own a coal bag
        if (BarsOres.COAL.getValue() < 230 && !Bank.isOpen() && CoalBag.getStock() > 0 || Inventory.contains(ItemID.COAL)) {
            log("Putting in coal");
        ...
        // take bars out
        if (!Inventory.isFull() && BarsOres.ADAMANTITE_BAR.getValue() > 0) {
            // take the bars out
        ...
        if (Bank.contains(ItemID.COAL) && Inventory.count(ItemID.COAL) < 12) {
            log("Withdraw coal");
            Bank.withdraw(ItemID.COAL, 12);
            Sleep.sleepUntil(() -> Inventory.count(ItemID.COAL) == 12, 2000);
            Bank.withdraw(ItemID.ADAMANTITE_ORE, 13);
            return ReactionGenerator.getNormal();
        }
        return ReactionGenerator.getNormal();
    }
