...
public class GoToCalvarion extends Fractal implements ChatListener {
    static Area TOMB_ENTRANCE = new Area(3176, 3686, 3182, 3680);
    ...
    public GoToCalvarion() {
        super(() -> !CALVARION_ARENA.contains(Players.getLocal()));
        Client.getInstance().addEventListener(this);

        this.inventoryLoadout = SettingsRepository.findInstanceOf(new CalvarionSettings()).loadout.loadout.setStrictSupplier(() -> !Combat.isInWild());
        this.equipmentLoadout = SettingsRepository.findInstanceOf(new CalvarionSettings()).loadout.equipmentLoadout;
    }
...
//                    Sleep.sleepUntil(() -> Worlds.getCurrentWorld() == world.getWorld(), 6400);
        ...
        if (entrance != null) {
            entrance.interact("Enter");
            Sleep.sleepUntil(() -> CombatUtil.getThreat() != null || TOMB_ENTRANCE.contains(Players.getLocal()),
                    1200);
        }
