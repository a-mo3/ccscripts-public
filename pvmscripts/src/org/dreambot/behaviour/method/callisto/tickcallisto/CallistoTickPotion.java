 ...
 */
public class CallistoTickPotion extends TickDecision implements ExperienceListener {
    public CallistoTickPotion() {
    ...
    public boolean evaluate() {
        boolean isOnEatDelay = GenericTickEat.lastAteTick == 0 || Client.getGameTick() - GenericTickEat.lastAteTick < 3;
        ...
        if (!isOnEatDelay && !isOnCombatDelay) {
            log("No delay active not an acceptable tick to pot up " + " Atk " + lastXpDrop + " Eat: " + GenericTickEat.lastAteTick);
            return false;
        }
