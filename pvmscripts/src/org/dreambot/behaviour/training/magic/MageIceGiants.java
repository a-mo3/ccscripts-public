...
public class MageIceGiants extends Fractal {
    ...
    public MageIceGiants(Supplier<Boolean> acceptCondition) {
        ...
        this.inventoryLoadout = new InventoryLoadout()
                ...
                .setEnabledCondition(() -> Skills.getRealLevel(Skill.MAGIC) >= 59 )
...
//                .setStrict(true)
    ...
    public int onLoop() {
        ...
        Spell s = Magic.getAutocastSpell();
        if (Skills.getRealLevel(Skill.DEFENCE) < defenceTarget) {
            if (!Magic.isAutocastDefensive() || s != getSpell()) {
                log("Def autocast");
                Magic.setDefensiveAutocastSpell(getSpell());
                return ReactionGenerator.getNormal();
            }
        } else {
            if (!Magic.isAutocasting() || s != getSpell()) {
                log("set autocast");
        ...
        GroundItem loot = GroundItems.closest(x -> x.getItem().isStackable() && ICE_DUNGEON.contains(x));
