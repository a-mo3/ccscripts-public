...
public class NightmareZone extends Fractal {
    public NightmareZone(Supplier<Boolean> acceptCondition, EquipmentLoadout loadout) {
    ...
    public Supplier<CombatStyle> styleSupplier = () -> {
        ...
        if (str < strMax && str <= Math.min(atk, def)) return CombatStyle.STRENGTH;
        if (atk < atkMax && atk <= def) return CombatStyle.ATTACK;
        return def < defMax ? CombatStyle.DEFENCE : CombatStyle.STRENGTH;
    };
    ...
    public int onLoop() {
        if (Client.isDynamicRegion()) {
            ...
            if (combatPot != null && Skills.getBoostedLevel(Skill.STRENGTH) <= Skills.getRealLevel(Skill.STRENGTH)) {
                log("Boost combats");
            ...
            if (activeOverload() == 0 && Skills.getBoostedLevel(Skill.HITPOINTS) > 50) {
                Item overload = ItemVariants.NMZ_OVERLOAD_POTION.getItem();
                if (overload != null) {
                    log("Sip overload");
                    overload.interact();
                    tabTimer.reset();
            ...
            if (activeOverload() > 1 && activeOverload() > 1 && Skills.getBoostedLevel(Skill.HITPOINTS) > 1) {
                log("Guzzle " + activeAbsorption() + " over: " + activeOverload());
...
//        }
        ...
        boolean useOverload = false;
        int overloads = storedOverloadCount() + ownedDoses("Overload");
        if (overloads < OVERLOAD_DOSE_TARGET) {
            log("Does not own enough overload doses");
            int requiredPoints = (OVERLOAD_DOSE_TARGET - overloads) * OVERLOAD_DOSE_COST;
            useOverload = requiredPoints < totalPoints;
            log("Requires overloads " + requiredPoints + " Can afford? " + useOverload);

            // if we have used points to decide to buy this take that from the total points counter
            if (useOverload) totalPoints -= requiredPoints;
        } else {
            useOverload = true;
        }
        ...
        if (getGPInCoffer() < 30_000) {
            ...
            if (coffer != null && coffer.distance() < 12) {
                log("Using coffer");
                coffer.interact();
        ...
        if (ItemVariants.SUPER_COMBAT_POTION.getItem() != null || ItemVariants.PRAYER_POTION.getItem() != null) {
            log("Bank potions");
            new BankAllInventoryEvent().execute();
            return ReactionGenerator.getNormal();
        }

        // todo consider mixing potions & dropping vials here
        // todo overloads n absorbs. from bank then barrels
        if (dosesInInventory("Overload") < OVERLOAD_DOSE_TARGET) {
            log("Get more overloads");
            if (ItemVariants.NMZ_OVERLOAD_POTION.getInventoryCount() < OVERLOAD_DOSE_TARGET / 4 && Bank.contains(x -> ItemVariants.NMZ_OVERLOAD_POTION.contains(x.getId()))) {
                log("Withdraw banked overloads");
                if (!Bank.isOpen()) {
                    log("Open bank");
                    if (Walking.shouldWalk()) Bank.open();
                    return ReactionGenerator.getNormal();
                ...
                Bank.withdraw(x -> ItemVariants.NMZ_OVERLOAD_POTION.contains(x.getId()), 8 - ItemVariants.NMZ_OVERLOAD_POTION.getInventoryCount());
                return ReactionGenerator.getNormal();
            }
            // purchase doses if required
            if (storedOverloadCount() < (OVERLOAD_DOSE_TARGET - dosesInInventory("Overload"))) {
                log("Needs to buy overload potion");
                buyDoses("Overload");
                return ReactionGenerator.getNormal();
            }
            // withdraw from barrel
            if (Dialogues.canEnterInput()) {
                int doses = OVERLOAD_DOSE_TARGET - dosesInInventory("Overload");
            ...
            GameObject overloadBarrel = GameObjects.closest("Overload potion");
            if (overloadBarrel == null) {
                log("Cant find overload barrel");
                return ReactionGenerator.getNormal();
            }
            overloadBarrel.interact("Take");
    ...
    private final int OVERLOAD_DOSE_TARGET = 32;

    private final int ABSORB_DOSE_COST = 1000;
    private final int OVERLOAD_DOSE_COST = 1500;

    private int storedAbsorbCount() {
        return PlayerSettings.getBitValue(3954);
    }

    private int storedOverloadCount() {
        return PlayerSettings.getBitValue(3953);
    }
    ...
    private int activeOverload() {
        return PlayerSettings.getBitValue(3955);
    }
}
