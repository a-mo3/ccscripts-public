    ...
    public static SandCrabs getRange(Supplier<Boolean> cond) {
        ...
        rangeCrabs.setInventoryLoadout(
                new InventoryLoadout()
                        .addItem(ItemID.JUG_OF_WINE, 1, 18)
                        .setRefill(1000)
...
//                .setEnabledCondition(() -> ScriptSettings.getSettingsData().useBoostPotions)
        ...
        rangeCrabs.paintArraySupplier = () -> {
            if (!Client.isLoggedIn()) return new String[0];
            return new String[]{
                    "tgt  " + Players.getLocal().getInteractingCharacter(),
                    "tgt me " + Players.getLocal().getCharacterInteractingWithMe()
            };
        };
        return rangeCrabs;
    }
