...
public class FletchLogs extends Fractal {
    ...
    public FletchLogs(int logId, int targetId, int quantity, int maxLvl) {
        // todo
        acceptCondition = (() -> {
            if (!ScriptSettings.fletchFromLogs()) return false;
    ...
    public FletchLogs(int logId, int targetId, int quantity, int maxLvl, Boolean isMagics) {
        acceptCondition = (() -> {
            if (!ScriptSettings.fletchFromLogs()) return false;
            if (Skills.getRealLevel(Skill.FLETCHING) > maxLvl) return false;
            if (!ScriptSettings.makeMagics() && isMagics) return false;
