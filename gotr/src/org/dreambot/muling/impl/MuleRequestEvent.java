...
public class MuleRequestEvent extends AbstractEvent implements ChatListener {
    ...
    private MuleConnection getMuleConnection() {
        ...
        if (muleConnection == null || !muleConnection.isOpen()) {
            attempts++;
            Logger.format("Connection attempt %d/%d", attempts, MAX_ATTEMPTS);
            if (attempts >= MAX_ATTEMPTS) setFailed(true);
            ...
            try {
                ...
                Logger.format("Failed to connect to mule server. make sure cCMule is running and addresses are configured correctly.");
                return null;
            }
        }
